...
## 3. Authentication Status (JWT)

**Active:** All ticket routes are now protected by JWT middleware. To access them, you must first log in:
1. Hit the `POST /api/auth/login` endpoint with a valid email and password.
2. Copy the `token` returned in the response.
3. For all other requests, go to the **Authorization** tab, select **Bearer Token**, and paste your token.

*(Pro-Tip: The Postman collection is configured with a test script on the Login route that automatically saves the token to a `{{jwt_token}}` variable. You can just put `{{jwt_token}}` in the Bearer Token box to automate this!)*
